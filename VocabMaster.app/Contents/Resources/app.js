...
const Parser = {
  ...
  buildEntry(term, raw) {
    ...
    return { id: uid(), term, raw, definitions, examples };
  },
...
function openWordModal(entry) {
  ...
  const html = `
    <div class="modal-term">
      ${entry.term}
      ${inWB ? '<span class="wb-badge">★ Saved</span>' : ''}
      <button class="btn-icon speak-btn" onclick="TTS.speak('${entry.term.replace(/'/g,"\\'")}')">🔊</button>
    </div>
    <div class="modal-section-label">Definition${entry.definitions.length > 1 ? 's' : ''}</div>
    ...
    <div class="modal-actions">
      <button class="btn btn-primary btn-sm" id="modalWbBtn">
        ${inWB ? '★ Remove from Wordbook' : '☆ Add to Wordbook'}
      </button>
    ...
    return `
      <div class="word-card" data-id="${entry.id}">
        ...
        <div class="word-card-actions">
          <button class="speak-btn" title="Pronounce" onclick="event.stopPropagation(); TTS.speak('${entry.term.replace(/'/g,"\\'")}')">🔊</button>
          <button class="star-btn ${starred ? 'starred' : ''}" title="${starred ? 'Remove from wordbook' : 'Save to wordbook'}"
...
function renderWordbook() {
  ...
  list.innerHTML = entries.map(entry => `
    <div class="wb-item">
      <div class="wb-term" onclick="openWordModal(Store.vocabulary.find(v=>v.id==='${entry.id}'))">
        ${entry.term}
      </div>
  ...
  document.querySelectorAll('.reader-word').forEach(span => {
    span.addEventListener('click', () => {
      ...
      if (id) {
        ...
        // Show "not in vocabulary" mini toast
        showToast(`"${word}" is not in your vocabulary`, 1800);
      }
    });
  });
}
